...
This is a sample for a larger population. The population would be then all movies of every genres ever created. Here we only have a selectio of movies and their raitings that try to represent a larger population.
...
The average revenues of all the movies in the given data is 88.846.315,00. Meanwhile the average revenue of all movies in the list staring Bruce Willis is 228.062.987,00 which lies over the average total mean.
...
The law of large number states that with a larger population sample, all values converge to a mean. For the Bruce Willis movies, not only do we have some NA values but we also only have 8 observations out of the 909 of the entire data set. If Bruce Willis had more movies in the list, the revenue mean would probably be closer to the average mean of the whole sample.
...
for (i in 1:100) {
  ...
  sample_vars[i] <- mean((movies_sample$profits - mean(movies_sample$profits))^2)
